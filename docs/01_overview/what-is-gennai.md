...
- **調査日:** 2026-07-08（実デプロイ反映: 2026-07-10）
- **ステータス:** 完了
- **はじめての人へ:** まず [START-HERE.md](./START-HERE.md) を読むと全体像が掴める。
...
**源内はAIそのものではなく「指揮者（オーケストレーター）」**。AI能力は持たず、内蔵チャットはLLM（Bedrock）を、業務アプリはExApp（外部AIアプリ）を呼び出して束ねるだけ。構成は直列ではなく、**内蔵チャットとExAppは別系統**である点に注意。

```
                 [ 利用者（職員） ]
                        │
                        ▼
        [ 源内 Web（genai-web）＝ 指揮者/ポータル ]
                        │
        ┌───────────────┴────────────────┐
        │ 内蔵チャットは直接               │ ExAppプロトコルで呼ぶ
        ▼                                  ▼
  [ LLM = Amazon Bedrock ]      [ ExApp（外部AIアプリ・別系統） ]
    Claude / Nova                 ・genai-ai-api のサンプル群
                                  ・自作アプリ（echo-observer 等）
                                  ※各ExAppはその先で自前のLLMを持つ
```

- 内蔵チャットのLLMは **Amazon Bedrock（Claude/Nova）で確定**（実デプロイで稼働確認済み）。
- `genai-ai-api` は源内本体の一部ではなく、**源内Webに登録して使うExApp（外部AIアプリ）のサンプル実装集**。
